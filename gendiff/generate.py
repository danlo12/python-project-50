def build_dicts_diff(content1, content2):
    ...
    for key_common in common:
        if isinstance(content1[key_common],dict) and isinstance(content2[key_common],dict):
            result.append({"type": "common_rec", "name_key": key_common, "value": build_dicts_diff(content1[key_common], content2[key_common])})
    ...
    for key_not_common in not_common:
        if key_not_common in content1:
            if isinstance(content1[key_not_common],dict):
                result.append({"type": "removed_rec", "name_key": key_not_common, "old_value": build_dicts_diff(content1[key_not_common], content1[key_not_common])})
            else:
            ...
            if type(content2[key_not_common]) is dict:
                result.append({"type": "added_rec", "name_key": key_not_common, "new_value": build_dicts_diff(content2[key_not_common], content2[key_not_common])})
            else:
