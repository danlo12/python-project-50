...
def stulish(result, lvl=2):
    ...
    for key in sorted(result, key=lambda k: k["name_key"]):
        if key["type"] == "common_rec":
            output.append(" " * lvl + "  " + key["name_key"] + ": " + stulish(key["value"], lvl + 4))
        elif key["type"] == "common":
            output.append(" " * lvl + "  " + key["name_key"] + ": " + format_value(key["value"]))
        elif key["type"] == "updated1":
            output.append(" " * lvl + "- " + key["name_key"] + ": " + stulish(key["old_value"], lvl + 4))
            ...
            output.append(" " * lvl + "+ " + key["name_key"] + ": " + stulish(key["new_value"], lvl + 4))
